...
{	pushOutline("WARN","overwritever run!")
	_execute("./busybox.exe mv ./remoteVer ./localVer",nullFunction)
}
...
function execHelper()
{
	if (window.isLinux){
		try{
		_execute("./busybox.exe sh ./shell.sh",launch)
		}
		catch(err)
		{
			launch()
...
if (window.isLinux){
	_execute("chmod +x *.AppImage&& ./*lobby*.AppImage --appimage-extract-and-run",window.close)
}
else{
	_execute("./*.exe",window.close)
}
