...
{	pushOutline("WARN","Removing remote Ver!")
	if (isLinux){_execute(window.wd+"/busybox.exe"+" rm \'"+window.wd+"/remoteVer\' ",nullFunction)}
	else{
		_execute("start \"\" \""+window.wd+"\\busybox.exe\""+" rm \""+window.wd+"\\remoteVer\"",nullFunction)
		//console.log("start \"\" \""+window.wd+"\\busybox.exe\""+" mv \'"+window.wd+"/remoteVer\' \'"+window.wd+"/localVer\'",nullFunction)
	}
}
...
function execHelper()
{
	if (window.isLinux){
		...
		try{
			_execute("start \"\" \""+window.wd+"\\shell.bat\"",launch)
		}
		catch(err)
		{
			//launch()
